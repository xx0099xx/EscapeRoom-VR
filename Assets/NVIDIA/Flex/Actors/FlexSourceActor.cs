...
namespace NVIDIA.Flex
{
    ...
    public class FlexSourceActor : FlexActor
    {
        ...
        void UpdateParticles(FlexContainer.ParticleData _particleData)
        {
            ...
            //Debug.Log(_particleData.GetParticle(m_indices[10]));
            //if (count < 100)
            //{
            for (int i = 0; i < m_indices.Length; i +=10)
            {
                UnityEngine.Vector4 data = _particleData.GetParticle(m_indices[i]);
                //Debug.Log(data);
                if (data[0] > 1 && data[0] < 2 && data[1] < 2 && data[1] > 0.5 && data[2] < -4 && data[2] > -5 ) // && data[1] < 1  && data[2] < -4)
                {
                    m_fire = true;
                    break;
                    //Vector3 v = new Vector3(0, 0, 5);
                    //Debug.Log(data);
                    //Debug.Log(m_ages[m_indices[i]]);
                    //_particleData.SetVelocity(m_indices[i], v);
                    //if (data[1] < 1)
                    //{
                    //    Debug.Log(data);
                    //    m_fire = true;
